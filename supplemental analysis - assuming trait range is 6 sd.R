# six.sd <- data.frame(log.size, log.size.se, log.rw, log.rw.se, log.color, log.col.se)
# six.sd$binom <- names(log.size)
# write.csv(six.sd, 'lib.spp.6sd.csv')
...
six.dat <- read.csv('lib.spp.6sd.csv') # load data
rownames(six.dat) <- six.dat$binom 
phylo <- read.tree('lib.na.phylo.tre') 
dim(six.dat) # check to make sure the data loaded in okay. Should output 72 7
...
log.size <- six.dat$log.size
log.size.se <- six.dat$log.size.se
names(log.size) <- six.dat$binom
names(log.size.se) <- six.dat$binom
...
# Brownian Motion     early burst           white           Delta              OU 
      # -46.02120       -44.21265        20.47636       -44.22612       -44.21265 

###### patterns of rw diversification
log.rw <- six.dat$log.rw
log.rw.se <- six.dat$log.rw.se
names(log.rw) <- six.dat$binom
names(log.rw.se) <- six.dat$binom
...
aic.rw
# Brownian Motion     early burst           white           Delta              OU 
      # -161.3248       -162.1947       -126.2558       -161.5826       -159.3603 

###### patterns of color diversification

log.color <- six.dat$log.color
log.col.se <- six.dat$log.col.se
names(log.color) <- six.dat$binom
names(log.col.se) <- six.dat$binom
