...
twofive.dat <- read.csv('lib.spp.25se.csv') # load data
rownames(twofive.dat) <- twofive.dat$binom 
phylo <- read.tree('lib.na.phylo.tre') 
dim(twofive.dat) # check to make sure the data loaded in okay. Should output 72 7
...
log.size <- twofive.dat$log.size
log.size.se <- twofive.dat$log.size.se
names(log.size) <- twofive.dat$binom
names(log.size.se) <- twofive.dat$binom
...
# Brownian Motion     early burst           white           Delta              OU 
      # -46.01003       -44.20397        20.47887       -44.21700       -44.20397 

###### patterns of rw diversification
log.rw <- twofive.dat$log.rw
log.rw.se <- twofive.dat$log.rw.se
names(log.rw) <- twofive.dat$binom
names(log.rw.se) <- twofive.dat$binom
...
aic.rw
# Brownian Motion     early burst           white           Delta              OU 
      # -162.4378       -162.7093       -126.5662       -162.3576       -160.4378 

###### patterns of color diversification

log.color <- twofive.dat$log.color
log.col.se <- twofive.dat$log.col.se
names(log.color) <- twofive.dat$binom
names(log.col.se) <- twofive.dat$binom
