...
main.dat <- read.csv('lib.spp.main.dat.csv') # load data
rownames(main.dat) <- main.dat$binom 
phylo <- read.tree('lib.na.phylo.tre') 
dim(main.dat) # check to make sure the data loaded in okay. Should output 72 7
...
log.size <- main.dat$log.size
log.size.se <- main.dat$log.size.se
names(log.size) <- main.dat$binom
names(log.size.se) <- main.dat$binom
...
# Brownian Motion     early burst           white           Delta              OU 
      # -46.07344       -44.25333        20.46391       -44.26886       -44.25333 

###### patterns of rw diversification
log.rw <- main.dat$log.rw
log.rw.se <- main.dat$log.rw.se
names(log.rw) <- main.dat$binom
names(log.rw.se) <- main.dat$binom
...
aic.rw
# Brownian Motion     early burst           white           Delta              OU 
      # -156.0030       -159.0253       -125.0830       -157.2833       -154.2663 

###### patterns of color diversification

log.color <- main.dat$log.color
log.col.se <- main.dat$log.col.se
names(log.color) <- main.dat$binom
names(log.col.se) <- main.dat$binom
